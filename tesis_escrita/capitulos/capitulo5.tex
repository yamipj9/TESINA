...
El presente capítulo sintetiza los hallazgos principales de la comparación entre las NTC-acero 2017
y las NTC-acero 2023 para el diseño de placas base de acero. Las conclusiones se organizan de
...
La transición de la NTC-acero 2017 a la NTC-acero 2023 representa una evolución normativa
...
El aporte técnico más importante de la NTC-acero 2023 se concentra en el reconocimiento explícito del
...
La investigación también confirma que la NTC-acero 2023 no es simplemente más permisiva que la
...
normas bajo las mismas condiciones de carga, geometría y propiedades mecánicas. Para el caso semilla
evaluado ($P_u=10$ ton, $M_u=7$ ton$\cdot$m, $f'_c=280$ kg/cm$^2$, perfil IR~305$\times$74.5,
placa $50\times50$ cm), $t_{req}=47.7$ mm con NTC-acero 2017 y con NTC-acero 2023, resultado
verificado sobre 500 puntos del barrido paramétrico de flexocompresión.

El cambio de mayor alcance conceptual es la reubicación del diseño de placas base desde un apéndice
hacia un capítulo principal dentro de la NTC-acero 2023. Esta modificación no solo mejora la
...
También se concluye que la NTC-acero 2023 hace más explícita la interacción entre las normas de acero
...
con anclas suficientemente embebidas para movilizar un volumen de concreto confinado. Para el caso
semilla con $h_{ef}=30$ cm y estribos \#4@15~cm (2~ramas), $R_{t,2017}=22.84$ ton y
$R_{t,2023}=36.18$ ton (+58~\%); con estribos \#5@10~cm el incremento alcanza el 137~\%. Cuando
el dado no cuenta con refuerzo transversal, $R_{et}=0$ y ambas normas producen resultado idéntico.
...
anclas es limitado. En estos casos, una conexión que cumplía con la NTC-acero 2017 puede requerir
redimensionamiento bajo la NTC-acero 2023. En el caso evaluado con ancla A325 de 7/8'' y dado de
70~cm, la distancia disponible es $c_{disp}=15.0$~cm, que cumple NTC-acero 2017
($c_{min,2017}=13.3$~cm) pero no cumple NTC-acero 2023 ($c_{min,2023}=15.6$~cm), con una
deficiencia de 0.55~cm que obliga a redimensionar el dado.

El barrido asociado al factor $R_{ev}$ confirmó que la NTC-acero 2023 incrementa la resistencia al
...
mejorar el desempeño del dado ante cortante basal. Con $s_{est}=15$~cm, $R_{v,2017}=29.69$ ton
frente a $R_{v,2023}=60.86$ ton (+105~\%); al densificar a $s_{est}=10$~cm el incremento sube a
+158~\% ($R_{v,2023}=76.45$ ton).
...
NTC-acero 2017 y la NTC-acero 2023 aplicadas al diseño de placas base. Esta comparación permite
...
La tercera aportación es la identificación de una lectura técnica balanceada de la NTC-acero 2023. La
...
Se recomienda que el diseño de placas base bajo la NTC-acero 2023 no se limite a verificar el espesor
...
columna, en particular secciones tubulares y cajón, ya que la NTC-acero 2023 incorpora disposiciones
...
herramientas facilitaría la aplicación correcta de la NTC-acero 2023 y reduciría errores de
